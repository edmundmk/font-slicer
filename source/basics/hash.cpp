...
#if __i386__ || __x86_64__ || defined( _MSC_VER )
...
{
    return hash32( string, strlen( string ) );
}

hash32_t hash32( const void* data, size_t size )
{
    return SpookyHash::Hash32( data, size, 0 );
}

hash64_t hash64( const char* string )
{
    return hash64( string, strlen( string ) );
}
...
void hash_context::data( uint8_t i )
{
    data( &i, sizeof( i ) );
}

void hash_context::data( uint16_t i )
{
    data( &i, sizeof( i ) );
}

void hash_context::data( uint32_t i )
{
    data( &i, sizeof( i ) );
}

void hash_context::data( uint64_t i )
{
    data( &i, sizeof( i ) );
}

void hash_context::data( int8_t i )
{
    data( &i, sizeof( i ) );
}

void hash_context::data( int16_t i )
{
    data( &i, sizeof( i ) );
}

void hash_context::data( int32_t i )
{
    data( &i, sizeof( i ) );
}

void hash_context::data( int64_t i )
{
    data( &i, sizeof( i ) );
}

void hash_context::data( float f )
{
    data( &f, sizeof( f ) );
}

void hash_context::data( double f )
{
    data( &f, sizeof( f ) );
}


void hash_context::data( const char* string )
{
    data( string, strlen( string ) );
}

void hash_context::data( const void* data, size_t size )
{
    context->Update( data, size );
}

hash32_t hash_context::hash32()
{
    uint64 hash1;
    uint64 hash2;
    context->Final( &hash1, &hash2 );
    return (hash32_t)hash1;
}

hash64_t hash_context::hash64()
{
    uint64 hash1;
    uint64 hash2;
    context->Final( &hash1, &hash2 );
